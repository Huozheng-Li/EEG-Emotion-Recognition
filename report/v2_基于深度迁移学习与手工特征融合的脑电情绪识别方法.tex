...
本方案依次考察预训练、模型容量和手工特征三个关键组件，逐项量化每个设计决策对分类性能的边际贡献。

\textbf{预训练效果消融}考察跨数据集迁移学习是否在本任务中产生了可测量的正向增益。该消融的目的是验证方案设计阶段的一项隐含前提——即DEAP源域与竞赛目标域之间的EEG信号分布足够接近、预训练权重具有可迁移性——是否实际成立。若成立，迁移学习应成为后续优化的核心方向；若不成立，则需重新审视源域选择策略，而非简单地增加预训练数据量。

\textbf{模型容量消融}考察参数规模与泛化性能之间的定量关系。该消融的目的是直接检验本方案的核心判断：在小样本EEG场景下，模型容量的控制是否比架构的精细化设计更为关键。TSception（3,111,106参数）与EEGNet（2,834参数）的参数规模相差约1,100倍，二者在完全相同的输入、数据划分和训练配置下进行对比，容量是唯一的变量，为上述判断提供了严格的实验检验。

\textbf{手工特征贡献分析}考察390维特征空间内部的信息密度分布。与前两项通过移除组件来观测性能变化的消融不同，本项分析的目的是在特征层面识别哪些频段与通道的组合对情绪分类贡献最大，一方面为特征精简提供工程依据，另一方面以数据驱动的方式验证模型是否收敛至已知的情绪神经机制（如$\gamma$波段和$\alpha$波段的主导作用），即模型的高贡献特征是否与神经科学文献中独立确定的情绪相关频段和脑区一致。
...
\bibitem{eegnet}
Chen, K., Jing, H., Liu, Q., Ai, Q. and Ma, L. (2023). A Novel Caps-EEGNet Combined with Channel Selection for EEG-based Emotion Recognition. \textit{Biomedical Signal Processing and Control}, 86(Part C): 105312.
...
\bibitem{deap}
Mouazen, B., Benali, A., Chebchoub, N.T. et al. (2025). Enhancing EEG-based Emotion Detection with Hybrid Models: Insights from DEAP Dataset Applications. \textit{Sensors}, 25(6): 1827.

\bibitem{lightgbm}
Zeng, H., Yang, C., Zhang, H. et al. (2019). A LightGBM-based EEG Analysis Method for Driver Mental States Classification. \textit{Computational Intelligence and Neuroscience}, 2019(1): 3761203.
...
Coelli, S., Calcagno, A., Cassani, C.M. et al. (2024). Selecting Methods for a Modular EEG Pre-processing Pipeline: An Objective Comparison. \textit{Biomedical Signal Processing and Control}, 90: 105830.

\bibitem{butterworth}
Butterworth, S. (1930). On the Theory of Filter Amplifiers. \textit{Experimental Wireless and the Wireless Engineer}, 7: 536--541.

\bibitem{duan_de}
Yao, L., Lu, Y., Qian, Y. et al. (2024). High-Accuracy Classification of Multiple Distinct Human Emotions Using EEG Differential Entropy Features and ResNet18. \textit{Applied Sciences}, 14(14): 6175.

\bibitem{coan}
Luo, Y., Tang, M. and Fan, X. (2025). Meta Analysis of Resting Frontal Alpha Asymmetry as a Biomarker of Depression. \textit{npj Mental Health Research}, 4(1): 2.
